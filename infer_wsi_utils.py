...
def generate_patch_list(ano, roi, patch_size, stride):
    ...
    path_loader = data.DataLoader(infer_dataset, num_workers=31, batch_size=2048, shuffle=False, drop_last=False)
    path_list_new = []
    for keeps, loca in path_loader:
        ...
        for idx in range(len(keeps)):
            if keeps[idx] == 1:
                a = eval(loca[idx])
                b = [[a[0], a[1]], [a[0] + stride//2, a[1]], [a[0], a[1] + stride//2], [a[0] + stride//2, a[1] + stride//2]]
                path_list_new.extend(b)
...
class DatasetSerialPatch(data.Dataset):
    ...
    def __init__(self, slide, path_list, patch_size, shape_augs=None, input_augs=None):
        self.slide = slide
